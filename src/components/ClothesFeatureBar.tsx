import { Truck, Undo2, ShieldCheck, Sparkles, Ruler, BadgePercent, Headphones, Gift } from "lucide-react";
import { useMemo } from "react";

/**
 * Marquee feature strip tailored for clothing/fashion stores.
 * Same layout as FeatureBar but with apparel-relevant promises.
 */
export const ClothesFeatureBar = () => {
  const features = useMemo(
    () => [
      { icon: Truck, title: "توصيل سريع", subtitle: "لكل المناطق" },
      { icon: Undo2, title: "استبدال وإرجاع", subtitle: "خلال 14 يوم" },
      { icon: Ruler, title: "مقاسات دقيقة", subtitle: "دليل مقاسات مفصل" },
      { icon: ShieldCheck, title: "جودة مضمونة", subtitle: "أقمشة ممتازة" },
      { icon: Sparkles, title: "تشكيلات جديدة", subtitle: "كل أسبوع" },
      { icon: BadgePercent, title: "عروض موسمية", subtitle: "خصومات حقيقية" },
      { icon: Headphones, title: "دعم على واتساب", subtitle: "ردود سريعة" },
      { icon: Gift, title: "تغليف مميّز", subtitle: "جاهز للهدايا" },
    ],
    [],
  );
      ...
      >
        ...
        <div className="flex w-max animate-marquee-rtl hover:[animation-play-state:paused] motion-reduce:animate-none py-3">
          {track.map(({ icon: Icon, title, subtitle }, i) => (
            <div key={`${title}-${i}`} className="flex items-center gap-3 px-5 shrink-0" dir="rtl">
